...
const CATEGORY_RULES = [
  {
    names: ['Зарплата'], type: 'income',
    keywords: ['зарплата', 'salary', 'аванс', 'оплата труда'],
  },
  {
    names: ['Еда'], type: 'expense',
    keywords: [
      'еда', 'продукт', 'ресторан', 'кафе', 'coffee', 'market', 'pizza', 'food',
      'выгодно', 'vygodno', 'хлебник', 'khlebnik', 'bauta', 'самокат', 'samokat',
      'яндекс*доставка', 'yandex*dostavka', 'доставка еды',
    ],
  },
  {
    names: ['Транспорт'], type: 'expense',
    keywords: [
      'такси', 'яндекс go', 'uber', 'metro', 'транспорт', 'азс', 'gas', 'whoosh',
      'scooters', 'самокат аренда', 'runcharge', 'rasp.el', 'электросамокат',
    ],
  },
  {
    names: ['Жильё'], type: 'expense',
    keywords: ['жкх', 'аренда', 'квартира', 'дом', 'rent', 'utility', 'картплата', 'оплата услуг'],
  },
  {
    names: ['Развлечения'], type: 'expense',
    keywords: ['кино', 'театр', 'игра', 'steam', 'netflix', 'подписк', 'betboom', 'pari', 'букмекер'],
  },
  {
    names: ['Здоровье'], type: 'expense',
    keywords: ['аптека', 'врач', 'клиник', 'health', 'pharmacy', 'стоматолог'],
  },
]

export function suggestCategoryId(categories, transaction) {
  const description = transaction.note.toLowerCase()
  const available = categories.filter(item => item.kind === transaction.type || item.kind === 'both')

  for (const rule of CATEGORY_RULES) {
    if (rule.type !== transaction.type || !rule.keywords.some(keyword => description.includes(keyword))) continue
    const category = available.find(item => rule.names.includes(item.name))
    if (category) return category.id
  }

  // User-created categories also work when their name appears in the bank description.
  const namedCategory = available.find(item => description.includes(item.name.toLowerCase()))
  return namedCategory?.id ?? available.find(item => item.name === 'Прочее')?.id ?? null
}
