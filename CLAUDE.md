...
- Глобальный Календарь: слои-источники через calendar-selectors + тогглы по модулям (Задачи/Привычки/Тренировки/Питание/Сон/Финансы/Встречи). События с временем внутри дня сортируются по возрастанию: от самой ранней пары к самой поздней; в ячейке месяца видны до 4 событий, чтобы полный учебный день не скрывался за `+1`. Режимы «События» / «Занятость» переключают подробные события и сводку интервалов: занятость учитывает только записи с явными start/end, показывает свободные промежутки и запоминается в localStorage. В месяце все свободные интервалы выводятся отдельными переносимыми плашками без обрезки. Учебные перерывы `12:35–12:55` и `14:35–14:55` вычитаются из занятости; обычные разрывы между событиями также показываются свободными. Не ломать существующие слои при добавлении новых. CalendarModule имеет 3 таба: Календарь / Встречи / Ссылка записи. Публичная страница самозаписи — /book/[slug] (без авторизации, API-роут /api/book/[slug]). Слот-генерация — lib/booking/slots.js. Repo-слой: lib/db/bookingLinks.js, lib/db/availability.js, lib/db/meetings.js. Антидублирование встреч — btree_gist exclusion constraint + server-side re-validation (409 при гонке). Удаление booking_link: availability_rules каскадно удаляются (ON DELETE CASCADE), meetings.link_id → NULL (ON DELETE SET NULL) — гостевые брони сохраняются. После удаления UI сбрасывается в «нет ссылки», владелец создаёт новую с новым slug.
