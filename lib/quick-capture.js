...
function findDate(text) {
  ...
  if (/(^|\s)сегодня(?=\s|$|[,.])/i.test(text)) return getTodayStr()
  if (/(^|\s)завтра(?=\s|$|[,.])/i.test(text)) {
    today.setDate(today.getDate() + 1)
    return dateString(today)
  }
  const match = text.match(/(?:^|\s)(\d{1,2})\s+(января?|февраля?|марта?|апреля?|мая?|июня?|июля?|августа?|сентября?|октября?|ноября?|декабря?)(?=\s|$|[,.])/i)
...
function findTime(text) {
  const match = text.match(/(?:^|\s)(?:в\s+(\d{1,2})(?::(\d{2}))?|(?:(\d{1,2})\s*(?:час(?:а|ов)?|ч\.?)|(\d{1,2}):(\d{2})))(?=\s|$|[,.])/i)
...
export function parseQuickCapture(text) {
  ...
  const food = value.match(/(?:^|\s)(?:съел(?:а)?|поел(?:а)?|съели)\s+(.+?)(?=$|[.;]|\s+(?:купил(?:а)?|сделал(?:а)?\s+тренировк|выполнил(?:а)?))/i)
  if (food) {
  ...
  if (/(^|\s)(купил(?:а)?|заплатил(?:а)?|потратил(?:а)?)(?=\s|$)/i.test(value)) {
  ...
  const workout = value.match(/(?:^|\s)(?:сделал(?:а)?\s+)?(тренировк\S*|пробежк\S*|бег|йог\S*|плавани\S*)(?=\s|$|[,.])/i)
  if (workout) {
  ...
  const task = value.match(/(?:^|\s)(?:выполнил(?:а)?|закончил(?:а)?|сделал(?:а)?)\s+(?:задач[ау]\s*)?(.+)/i)
