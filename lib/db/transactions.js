...
function toInsertRow(userId, { type, amount, categoryId = null, note = '', date, importHash = null }) {
  return {
    user_id:     userId,
    type:        type       || 'expense',
    amount:      amount     || 0,
...
export const transactionsRepo = {
  ...
  async create(userId, { type, amount, categoryId = null, note = '', date, importHash = null }) {
    const { data, error } = await supabase
      .from('transactions')
      .insert(toInsertRow(userId, { type, amount, categoryId, note, date, importHash }))
      .select().single()
    if (error) throw error
    return fromDB(data)
  },

  async createMany(userId, transactions) {
    const { data, error } = await supabase
      .from('transactions')
      .insert(transactions.map(transaction => toInsertRow(userId, transaction)))
      .select()
    if (error) throw error
    return (data ?? []).map(fromDB)
  },
