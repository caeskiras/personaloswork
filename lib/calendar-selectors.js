  ...
  return meetings
    ...
    .map(m => {
      ...
      return {
        ...
        endMinutes: end && !Number.isNaN(end.getTime())
...
// The timetable prints the third and fourth pairs as two lesson blocks with
// a longer break in the middle. Keep these gaps free in availability mode.
const TIMETABLE_BREAKS = [
  { start: 12 * 60 + 35, end: 12 * 60 + 55 },
  { start: 14 * 60 + 35, end: 14 * 60 + 55 },
]

function splitAroundTimetableBreaks(event) {
  let ranges = [{ start: event.startMinutes, end: event.endMinutes, event }]
  for (const pause of TIMETABLE_BREAKS) {
    ranges = ranges.flatMap(range => {
      if (range.start < pause.start && range.end > pause.end) {
        return [
          { ...range, end: pause.start },
          { ...range, start: pause.end },
        ]
      }
      return [range]
    })
...
/**
 * Builds a compact availability summary from events that have explicit times.
 * Overlapping events are merged; real gaps remain visible as free windows.
 */
export function buildBusySummary(events) {
  const ranges = events
    .filter(e => Number.isFinite(e.startMinutes) && Number.isFinite(e.endMinutes) && e.endMinutes > e.startMinutes)
    .flatMap(splitAroundTimetableBreaks)
    .sort((a, b) => a.start - b.start || a.end - b.end)

  if (ranges.length === 0) {
    return { ranges: [], freeWindows: [], firstStart: null, lastEnd: null, totalBusyMinutes: 0 }
  }
  ...
  for (const range of ranges) {
    ...
    if (last && range.start <= last.end) {
      ...
      last.events.push(range.event)
    } else {
      merged.push({ start: range.start, end: range.end, events: [range.event] })
    }
  }

  const freeWindows = []
  for (let i = 1; i < merged.length; i++) {
    freeWindows.push({ start: merged[i - 1].end, end: merged[i].start })
  }
  ...
  return {
    ...
    lastEnd: merged[merged.length - 1].end,
    totalBusyMinutes: merged.reduce((sum, range) => sum + range.end - range.start, 0),
  }
}
...
export function buildDayMap(events) {
  ...
  for (const dayEvents of map.values()) {
    dayEvents.sort((a, b) => (a.sortTime ?? Number.MAX_SAFE_INTEGER) - (b.sortTime ?? Number.MAX_SAFE_INTEGER))
  }
  return map
}
